/*
 * jQuery autosuggest plugin 2.32
 *
...
(function($) {
  ...
  $.widget( "solr.autosuggest", $.ui.autocomplete, {
    options: {
      ...
      minLength: 3,
      extraParams: {
        fields: "topic_search,container_title,title,type,thumbnail,icon,url,field_Telephone_s,field_Phone_s,field_Mobile_s"
      },
      limits: {
        "global": 3,
        "persons": 3,
      ...
      templates: {
        "persons": "<li class='ui-autosuggest-item {{:group}} {{:isFirst}} {{:isLast}}'>{{:header}}"+
            "{{if phoneNumber}}<a class='ui-autosuggest-phone-number fa fa-phone i18n' href='{{:phoneSchema}}:{{:phoneNumber}}'></a>{{/if}}"+
            "<a href='{{:url}}' class='ui-autosuggest-link'>"+
              ...
              "</tr></table></a>"+
            "</li>",
        "other": "<li class='ui-autosuggest-item {{:group}} {{:isFirst}} {{:isLast}}'>{{:header}}"+
            "<a href='{{:url}}' class='ui-autosuggest-link'>"+
              "<table class='foswikiNullTable'><tr>"+
                "<th><div>{{:thumbnail}}</div></th>"+
      ...
      _renderItem: function(ul, item) {
        ...
        if (!item._done) {
          ...
          if (typeof(item.thumbnail) !== 'undefined') {
            // icons
            if (/^ma-/.test(item.thumbnail)) {
              item.thumbnail = "<i class='icon ma "+item.icon+"' />";
            } else if (/^fa-/.test(item.thumbnail)) {
              item.thumbnail = "<i class='icon fa "+item.icon+"' />";
            } else {
              // images
              if (/^(\/|https?:)/.test(item.thumbnail)) {
                item.thumbnailUrl = self.options.thumbnailBase + '&file=' + item.thumbnail;
              } else if (/\.svgz?$/.test(item.thumbnail)) {
                item.thumbnailUrl = foswiki.getPubUrlPath(item.web, item.topic, item.thumbnail);
              } else {
